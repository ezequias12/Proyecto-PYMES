...
router.get("/api/categoriasmock", async function (req, res) {
  ...
  if (Nombre) {
    // Filtrar categorías cuyo nombre coincida
    const resultado = arr_CategoriasMock.filter((c) =>
      c.Nombre.toLowerCase().includes(Nombre.toLowerCase())
    );
    res.json(resultado);
  } else {
    // Si no se pasa el parámetro, devuelve todo
...
router.post("/api/categoriasmock/", (req, res) => {
  const { Nombre } = req.body; //se podria escribir const Nombre = req.body.Nombre;
  let categoria = {
    Nombre,
